...
allowed-tools: planner, filesystem, git, fetch, convert, flutter, dart, code-index
context: fork
agent: task-agent
---

Always start a step by running /code-index skill to index new and updated code files in the project.

Use code-index tool search operation when exploring the code base.

Use the planner tool to fetch the task by task id.

The returned task should have state todo or started, otherwise stop and inform the user that the task has the wrong status.
...
A parent task should be prefixed with "Parent:" in the title. Each step in a parent task should contain sub-task title and id.

When you perform a task, it is important that you follow the task process:

1. change status of the task to started
2. (skip if parent task)if we will edit files, use git to create a git branch
...
8. (skip if parent task) if files have been edited, use git to commit the changes to the git branch
9. when all the steps are done, change status of task to done
10. (skip if parent task) if we are on a git branch, use git to merge to master and change task status to merged
