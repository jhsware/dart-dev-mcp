...
void main() {
  ...
  group('worktree canonicalization', () {
    test('worktree checkout root maps to the main repo store', () {
      ...
      expect(
        canonicalProjectPath(worktree.path),
        canonicalProjectPath(repo.path),
      );
      expect(dataDirNameFor(worktree.path), dataDirNameFor(repo.path));
      expect(
        dbPathFor(tmp.path, worktree.path),
        dbPathFor(tmp.path, repo.path),
      );
    });

    test(
      'sub-package inside a worktree maps to the main sub-package store',
      () {
        final repo = Directory(p.join(tmp.path, 'repo'))..createSync();
        final pkg = Directory(p.join(repo.path, 'pkg'))..createSync();
        final worktreePkg = Directory(
          p.join(repo.path, '.worktrees', 'my-branch', 'pkg'),
        )..createSync(recursive: true);

        expect(
          canonicalProjectPath(worktreePkg.path),
          canonicalProjectPath(pkg.path),
        );
    ...
    test('non-worktree paths resolve as before', () {
      ...
      expect(
        canonicalProjectPath(proj.path),
  ...
  group('fresh-checkout rescan (reset mtimes)', () {
    ...
    test(
      'populated store: full re-hash once, stat short-circuit afterwards',
      () async {
        final project = Directory(p.join(tmp.path, 'proj'))..createSync();
        const fileCount = 200;
        for (var i = 0; i < fileCount; i++) {
          File(
            p.join(project.path, 'f$i.yaml'),
        ...
        ScanOperations ops() =>
            ScanOperations(database: db, workingDir: project, dbPath: dbPath);
        ...
        final storedMtime =
            db.select('SELECT mtime FROM files WHERE path = ?', [
                  'f0.yaml',
                ]).first['mtime']
                as String;
        expect(
          DateTime.parse(storedMtime).toUtc(),
        ...
        // ignore: avoid_print
        print(
          'rescan of $fileCount identical files: '
