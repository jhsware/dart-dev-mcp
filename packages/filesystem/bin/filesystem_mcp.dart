...
///
/// Usage: `dart run bin/filesystem_mcp.dart --project-dir=PATH1 [--project-dir=PATH2 ...]`
void main(List<String> arguments) async {
  ...
  logInfo('filesystem', 'Filesystem MCP Server starting...');
  logInfo('filesystem', 'Project dirs: ${serverArgs.projectDirs.join(", ")}');

  final server = McpServer(
    Implementation(name: 'jhsware_code_filesystem', version: '1.0.0'),
...
- If provided with endLine: replaces lines startLine to endLine''',
  ...
  logInfo('filesystem', 'Filesystem MCP Server running on stdio');
}

void _printUsage() {
  stderr.writeln(
    'Usage: filesystem_mcp --project-dir=PATH1 [--project-dir=PATH2 ...]',
  );
  stderr.writeln('');
  ...
  try {
    switch (operation) {
      ...
      case 'edit-file':
        ...
        return await writeOps.editFile(
          path,
          content,
          startLine,
          endLine,
    ...
    return errorResult('filesystem:$operation', e, stackTrace, {
      'operation': operation,
      'path': path,
    });
  }
