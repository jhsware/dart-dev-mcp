...
void main() {
  ...
  group('editFile insert_at parameter', () {
    ...
    test('combined with startLine is rejected and file is untouched', () async {
      final file = File('${libDir.path}/foo.dart');
      await file.writeAsString('line1\nline2\n');
  ...
  group('editFile overwrite mode', () {
    test('without overwrite:true is rejected and file is untouched', () async {
      final file = File('${libDir.path}/foo.dart');
    ...
    test(
      'with overwrite:true replaces the file and reports the delta',
      () async {
        final file = File('${libDir.path}/foo.dart');
  ...
  group('extractLines', () {
    ...
    test('appending into existing dest with trailing-newline source', () async {
      final src = File('${libDir.path}/src.dart');
      await src.writeAsString('a\nb\n');
      final dest = File('${libDir.path}/dest.dart');
    ...
    test(
      'preserves source trailing newline when removeFromSource=true',
      () async {
        final src = File('${libDir.path}/src.dart');
        await src.writeAsString('a\nb\nc\nd\n');
