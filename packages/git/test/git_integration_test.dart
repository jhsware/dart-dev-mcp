...
void main() {
  group('Git Integration Tests', () {
    ...
    test('commit works in restricted environment', () async {
      ...
      result = await Process.run(
        'git',
        ['commit', '--no-gpg-sign', '-m', 'Add main.dart'],
        workingDirectory: repoDir.path,
        environment: restrictedEnv,
      );
    ...
    test('branch-create auto-switches to the new branch', () async {
      ...
      expect(
        result.exitCode,
        0,
        reason: 'checkout -b failed: ${result.stderr}',
      );
      ...
      expect(
        result.exitCode,
        0,
        reason: 'checkout -b with from failed: ${result.stderr}',
      );
  ...
  group('SSH Signing Tests', () {
    ...
    setUpAll(() async {
      ...
      if (gitVersion.exitCode == 0) {
        ...
        print(
          'Git version: $versionStr (SSH signing: ${gitSupportsSSH ? "supported" : "not supported"})',
        );
      } else {
        gitSupportsSSH = false;
      }
    ...
    setUp(() async {
      ...
      final keygenResult = await Process.run('ssh-keygen', [
        '-t',
        'ed25519',
        '-f',
        sshKeyPath,
    ...
    test('can commit with SSH signing using -c config options', () async {
      ...
      result = await Process.run(
        ...
        [
          ...
          'gpg.format=ssh',
          '-c',
          'user.signingkey=$sshKeyPath.pub',
          'commit',
          '-S',
          '-m',
          'SSH signed commit',
      ...
      expect(
        result.exitCode,
        0,
        reason: 'SSH-signed commit failed: ${result.stderr}',
      );
    ...
    test(
      'can configure allowed_signers for SSH signature verification',
      () async {
        if (!gitSupportsSSH) {
          markTestSkipped('Git does not support SSH signing (requires 2.34+)');
          return;
        ...
        await File(
          allowedSignersPath,
        ).writeAsString('test@example.com $pubKey');
    ...
    test('SSH signing fails gracefully without valid key', () async {
      ...
      final result = await Process.run(
        ...
        [
          ...
          'gpg.format=ssh',
          '-c',
          'user.signingkey=/nonexistent/key.pub',
          'commit',
          '-S',
          '-m',
          'This should fail',
      ...
      expect(
        result.exitCode,
        isNot(0),
        reason: 'Commit should fail with invalid key',
      );
    });
  });
  ...
  group('SSH Agent Signing Tests', () {
    ...
    setUp(() async {
      ...
      sshKeyPath = p.join(sshDir.path, 'id_ed25519');
      var keygenResult = await Process.run('ssh-keygen', [
        '-t',
        'ed25519',
        '-f',
        sshKeyPath,
      ...
      if (keygenResult.exitCode != 0) {
        print(
          'Failed to generate SSH key with passphrase: ${keygenResult.stderr}',
        );
        return;
      }
      ...
      final sockMatch = RegExp(
        r'SSH_AUTH_SOCK=([^;]+)',
      ).firstMatch(agentOutput);
    ...
    test('can start ssh-agent and add passphrase-protected key', () async {
      ...
      expect(
        listResult.exitCode,
        0,
        reason: 'ssh-agent should have keys loaded',
      );
      expect(
        listResult.stdout,
        contains('ED25519'),
        reason: 'Our test key should be in agent',
      );
    });

    test(
      'can sign commits using ssh-agent with passphrase-protected key',
      () async {
        if (!gitSupportsSSH) {
          markTestSkipped('Git does not support SSH signing (requires 2.34+)');
          return;
...
''';
        ...
        await testFile.writeAsString(
          'This commit uses ssh-agent for signing\n',
        );
        ...
        expect(
          result.exitCode,
          0,
          reason: 'SSH-signed commit via agent failed: ${result.stderr}',
        );
    ...
    test(
      'SSH signing fails without agent for passphrase-protected key',
      () async {
        if (!gitSupportsSSH) {
          markTestSkipped('Git does not support SSH signing (requires 2.34+)');
          return;
        ...
        final result = await Process.run(
          ...
          [
            ...
            '-S',
            '-m',
            'This should fail without agent',
          ],
          ...
          environment: {
            ...
            'USER': Platform.environment['USER'] ?? 'test',
            'SSH_ASKPASS':
                '/bin/false', // Use false to immediately fail password prompts
            'SSH_ASKPASS_REQUIRE':
                'force', // Force SSH to prompt for passphrase using SSH_ASKPASS
            // No SSH_AUTH_SOCK!
          },
        );
        ...
        expect(
          ...
          isNot(0),
          reason:
              'Commit should fail without SSH_AUTH_SOCK for passphrase-protected key',
        );
        expect(
          result.stderr,
          contains('passphrase'),
    ...
    test('verifies SSH_AUTH_SOCK environment passing', () async {
      ...
      var result = await Process.run('ssh-add', [
        '-l',
      ], environment: envWithAgent);
      print(
        'With SSH_AUTH_SOCK: exit=${result.exitCode}, keys=${result.stdout}',
      );
      ...
      print(
        'Without SSH_AUTH_SOCK: exit=${result.exitCode}, stderr=${result.stderr}',
      );
      expect(
        result.exitCode,
        2,
        reason: 'Should fail to connect without SSH_AUTH_SOCK',
      );
    });
  });
  ...
  group('GPG Signing Tests', () {
    ...
    setUpAll(() async {
      ...
      if (gpgAvailable) {
        ...
        print(
          'GPG version: ${(versionResult.stdout as String).split('\n').first}',
        );
      } else {
...
''';
      ...
      final genResult = await Process.run(
        ...
        environment: {...Platform.environment, 'GNUPGHOME': gnupgDir.path},
      );
    ...
    test('can commit with GPG signing', () async {
      ...
      await Process.run(
        'git',
        ['config', 'gpg.format', 'openpgp'], // Explicitly use GPG, not SSH
        workingDirectory: repoDir.path,
        environment: gpgEnv,
      );
      await Process.run(
        'git',
        ['config', 'user.signingkey', testKeyId!],
        workingDirectory: repoDir.path,
        environment: gpgEnv,
      );

      final gpgPath = (await Process.run('which', [
        'gpg',
      ])).stdout.toString().trim();
      await Process.run(
        'git',
        ['config', 'gpg.program', gpgPath],
        workingDirectory: repoDir.path,
        environment: gpgEnv,
      );
      ...
      expect(
        result.exitCode,
        0,
        reason: 'GPG-signed commit failed: ${result.stderr}',
      );
  ...
  group('Git MCP Server Simulation', () {
    ...
    test('simulates commit with sign="none"', () async {
      ...
      result = await Process.run(
        'git',
        ['commit', '--no-gpg-sign', '-m', 'Unsigned commit'],
        workingDirectory: repoDir.path,
        environment: Platform.environment,
      );
  ...
  group('Monorepo layout (git root != project dir)', () {
    ...
    setUp(() async {
      ...
      await File(p.join(repoDir.path, 'README.md')).writeAsString('# Monorepo');

      // Create a file inside sub-project
      await File(p.join(subLibDir, 'foo.dart')).writeAsString('void foo() {}');
