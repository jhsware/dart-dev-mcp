  ...
  group('Planner Database Tests', () {
    ...
    group('Task Operations', () {
      ...
      test('can update task memory', () {
        ...
        db.execute('UPDATE tasks SET memory = ?, updated_at = ? WHERE id = ?', [
      ...
      test('can query tasks by status', () {
        ...
        final todoTasks = db.select('SELECT * FROM tasks WHERE status = ?', [
          'todo',
        ]);
        expect(todoTasks, hasLength(1));
        expect(todoTasks.first['title'], 'Todo Task');

        final doneTasks = db.select('SELECT * FROM tasks WHERE status = ?', [
          'done',
        ]);
      ...
      test('validates task statuses including draft and merged', () {
        ...
        for (final status in validStatuses) {
          ...
          final result = db.select('SELECT status FROM tasks WHERE id = ?', [
            id,
          ]);
      ...
      test('can filter tasks by backlog status', () {
        ...
        final backlogTasks = db.select('SELECT * FROM tasks WHERE status = ?', [
          'backlog',
        ]);
        expect(backlogTasks, hasLength(2));
        expect(
          backlogTasks.every((t) => t['title'].toString().contains('Backlog')),
          isTrue,
        );
      });
    });
    ...
    group('Step Operations', () {
      ...
      test('can update step status', () {
        ...
        db.execute('UPDATE steps SET status = ?, updated_at = ? WHERE id = ?', [
          'done',
          now,
          stepId,
        ]);

        final result = db.select('SELECT status FROM steps WHERE id = ?', [
          stepId,
        ]);
      ...
      test(
        'backward compatibility: legacy step statuses can be stored and read',
        () {
      ...
      test('can get task with its steps', () {
        ...
        // Get task
        final taskResult = db.select('SELECT * FROM tasks WHERE id = ?', [
          taskId,
        ]);
    ...
    group('Project Instructions', () {
      test('can read instructions file when it exists', () async {
        final instructionsPath = p.join(aiToolDir.path, 'AGENTS.md');
...
''';
    ...
    group('Complex Workflows', () {
      ...
      test('can create a task with multiple steps and update statuses', () {
        ...
        // Verify state
        final taskResult = db.select('SELECT status FROM tasks WHERE id = ?', [
          taskId,
        ]);
...
''';
        ...
        final result = db.select('SELECT memory FROM tasks WHERE id = ?', [
          taskId,
        ]);
      ...
      test('can handle concurrent projects', () {
        ...
        for (final projectId in projects) {
          final tasks = db.select('SELECT * FROM tasks WHERE project_id = ?', [
            projectId,
          ]);
    ...
    group('Edge Cases', () {
      ...
      test('handles special characters in text fields', () {
        ...
        const specialTitle =
            "Task with 'quotes', \"double quotes\", and émojis 🎉";
...
''';
      ...
      test('preserves timestamp precision', () {
        ...
        final result = db.select(
          'SELECT created_at, updated_at FROM tasks WHERE id = ?',
          [taskId],
        );
